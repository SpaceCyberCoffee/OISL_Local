...
static int32 Generic_ADCS_AppInit(void)
{
    ...
    */
    ...
    status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(OISL_DEVICE_TLM_MID), Generic_ADCS_AppData.CmdPipe);
    if (status != CFE_SUCCESS)
    {
        CFE_ES_WriteToSysLog("Generic_ADCS App: Error Subscribing to OISL_DEVICE_TLM_MID, RC = 0x%08lX\n", (unsigned long)status);
        return (status);
    }
    ...
    switch (CFE_SB_MsgIdToValue(MsgId))
    {
        ...
        */
        ...
        case GENERIC_STAR_TRACKER_DEVICE_TLM_MID:
            ...
            break;
        case OISL_DEVICE_TLM_MID:
            Generic_ADCS_ingest_generic_OISL(Generic_ADCS_AppData.MsgPtr, &Generic_ADCS_AppData.DIPacket.Payload.OISL);
            break;

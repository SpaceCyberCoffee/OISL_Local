      ...
      for(Ist=0;Ist<S->Nst;Ist++) {
         ...
         if (ST->SampleCounter >= ST->MaxCounter) {
            ...
            if (ST->Valid) {
               ...
               double[4] qCN = rotationMatrixToQuaternion(S->B[0].CN);
               ...
               // Store the quaternion with noise
               QxQ(Qnoise, qCN, ST->qn);
            }
...
void rotationMatrixToQuaternion(double R[3][3], double[4] q) {
    double[4] q;

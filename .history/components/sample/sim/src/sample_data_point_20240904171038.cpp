    ...
    void SampleDataPoint::do_parsing(void) const
    {
        if (_42_parsing)
        {
            try {
                ...
                */
                std::string key;
                key.append("SC[").append(std::to_string(_sc)).append("].svn"); // SC[N].svb
                ...
                /* Debug print */
                sim_logger->debug("SampleDataPoint::SampleDataPoint:  Parsed ECI POSITION forward = %f %f %f", _sample_data[0], _sample_data[1], _sample_data[2]);

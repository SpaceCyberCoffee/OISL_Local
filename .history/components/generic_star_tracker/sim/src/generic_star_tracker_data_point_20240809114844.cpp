...
namespace Nos3
{
    ...
    void Generic_star_trackerDataPoint::do_parsing(void) const
    {
        try {
            ...
            */
            std::string valid_key, qn_key, qn_key_old;
            valid_key.append("SC[").append(std::to_string(_sc)).append("].AC.ST[").append(std::to_string(_st)).append("]"); // SC[N].AC.ST[M].qn / Valid
            qn_key_old = valid_key;
            qn_key.append("SC[").append(std::to_string(_sc)).append("].B[0]");
            valid_key.append(".Valid");
            qn_key.append(".qn");
            qn_key_old.append(".qn");

            /* Parse 42 telemetry */
            std::string valid_value = _dp.get_value_for_key(valid_key);
            std::string qn_values_seri = _dp.get_value_for_key(qn_key);
            sim_logger->debug("I retrieved this stuff: %s", qn_values_seri.c_str());

            std::string qn_values = _dp.get_value_for_key(qn_key_old);


            /* MK - added to get quaternions */
            std::string keyq;
            keyq.append("SC[").append(std::to_string(_sc)).append("].B[0].qn");
            std::string quat_text = _dp.get_value_for_key(keyq);
            std::vector<double> quat_data;
            quat_data.clear();
            std::istringstream iss(quat_text);
            for (std::string s; iss >> s; )
                quat_data.push_back(std::stod(s));
            sim_logger->debug("I retrieved this stuff: %f", qn_values_seri.c_str());
            ...
            /* Debug print */
            sim_logger->debug("Generic_star_trackerDataPoint::Generic_star_trackerDataPoint:  Parsed qn = %f %f %f %f", _generic_star_tracker_data[0], _generic_star_tracker_data[1], _generic_star_tracker_data[2], _generic_star_tracker_data[3]);

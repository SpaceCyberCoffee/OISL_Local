...
/*
** Request data command THIS IS NOT THE ONE
*/
...
void NOVATEL_OEM615_ParseBestXYZA(NOVATEL_OEM615_Device_Data_tlm_t* device_data_struct)
{
    ...
    // OS_printf("Called this ParseBest method, so update the GPS data. \n ");
    ...
    if (token != NULL) device_data_struct->ECIX = atof(token);
    ...
    for (int i = 0; i < 3; i++) {
        fprintf(fp, "%f ", ECI_vector[i]);
    }
    // close the file
    fclose(fp);   

    double ECI_vel_vector [] = {device_data_struct->VelX, device_data_struct->VelY, device_data_struct->VelZ};
    char *filename2 = "/home/jstar/Desktop/github-nos3/components/oisl/fsw/src/ECI_velocity.txt";

    // open the file for writing
    FILE *fp2 = fopen(filename2, "a");
    if (fp2 == NULL)
    {
        OS_printf("Error opening the file %s", filename2);
    }
    // write to the text file
    for (int i = 0; i < 3; i++) {
        fprintf(fp2, "%f ", ECI_vel_vector[i]);
    }
    fprintf(fp2, "\n");
    // close the file
    fclose(fp2);   

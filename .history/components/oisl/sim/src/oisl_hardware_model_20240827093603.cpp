...
namespace Nos3
{
    ...
    void OislHardwareModel::create_oisl_data(std::vector<uint8_t>& out_data)
    {
        ...
        */
        ...
        const char *filenametest = "/home/jstar/Desktop/github-nos3/sims/build/bin/forward_stability_2vectorsPT3.txt";

        // Add the errors
        FILE *file_ISL_change = fopen("/home/jstar/Desktop/github-nos3/sims/build/bin/ERRORS.txt", "r");
        double errors[3];  // Default value
        if (file_ISL_change != NULL) {
            fscanf(file_ISL_change, "%lf %lf %lf", &errors[0], &errors[1], &errors[2]);
            fclose(file_ISL_change);
        }

        // open the file for writing
        FILE *fp3 = fopen(filenametest, "a");
        // write to the text file
        fprintf(fp3, "%u %f %f %f %f\n", forward_alignment, VoV_F, errors[0], errors[1], errors[2]);
        // close the file
        fclose(fp3);   

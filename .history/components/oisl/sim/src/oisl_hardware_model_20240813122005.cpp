...
namespace Nos3
{
    ...
    _enabled(OISL_SIM_SUCCESS), _count(0), _config(0), _status(0)
    {
        ...
        if (remove(VoV_forward_filename) == 0) {
            sim_logger->debug("File VoV deleted successfully: %s\n", file_path);
        } else {
            sim_logger->debug("Failed to delete the file: %s\n", file_path);
        }
    ...
    void OislHardwareModel::create_oisl_data(std::vector<uint8_t>& out_data)
    {
        ...
        */
        ...
        FILE *file_VoV = fopen(VoV_forward_filename, "r");
        double VoV = 0.0;  // Default value

        if (file_VoV != NULL) {
            fscanf(file_VoV, "%lf", &VoV);
            fclose(file_VoV);
        } else {
            sim_logger->debug("File cannot be opened or does not exist. Setting x to default value: %f\n", VoV);
        }

        // Use the value of x (which is either read from the file or set to 0.0)
        sim_logger->debug("Hallooooo %f\n", VoV);

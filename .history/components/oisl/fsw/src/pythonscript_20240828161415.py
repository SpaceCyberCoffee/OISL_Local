...
if __name__ == "__main__":
    ...
    print(central_sat, len(central_sat), type(central_sat[0]))

    # Create the plot
    plt.figure(figsize=(10, 6))
    plt.plot(range(len(central_sat)), central_sat, label='Central SAT')
    plt.plot([i + 237 for i in range(len(forward_sat))], forward_sat, label='Forward SAT')
...
def calculate_orbital_period(altitude_km):
    ...
    orbital_period_seconds = 2 * math.pi * math.sqrt(((earth_radius_km + altitude_km)**3) / (gravitational_constant * earth_mass_kg))

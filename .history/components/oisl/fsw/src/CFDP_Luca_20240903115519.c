...
CF_CFDP_PduAck_t createAck(CF_CFDP_FileDirective_t dir_code, CF_CFDP_ConditionCode_t cc, int segmentNumber) {
    ...
    ack.directive_and_subtype_code = (CF_CFDP_uint8_t)((dir_code << 4) | 1); // Directive and subtype code
    ack.cc_and_transaction_status = (CF_CFDP_uint8_t)((cc << 4) | CF_CFDP_TransactionStatus_SUCCESS); // Condition code and transaction status

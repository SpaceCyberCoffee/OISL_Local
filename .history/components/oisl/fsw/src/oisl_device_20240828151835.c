...
void get_isl_vectors(double* forward_ISL_vector, double* backward_ISL_vector)
{
    ...
    // TEST TO BE DELITED
    char *filename_test_central = "/home/jstar/Desktop/github-nos3/components/oisl/fsw/src/central_test.txt";
    char *filename_test_for = "/home/jstar/Desktop/github-nos3/components/oisl/fsw/src/forward_test.txt";

    // open the file for writing
    FILE *fp_c = fopen(filename_test_central, "w");
    if (fp_c == NULL)
    {
        OS_printf("Error opening the file %s", filename_test_central);
    }
    // write to the text file
    for (int i = 0; i < 3; i++) {
        fprintf(fp_c, "%f ", central_sat[i]);
    }
    // close the file
    fclose(fp_c);   
    // open the file for writing
    FILE *fp_f = fopen(filename_test_for, "w");
    if (fp_f == NULL)
    {
        OS_printf("Error opening the file %s", filename_test_for);
    }
    // write to the text file
    for (int i = 0; i < 3; i++) {
        fprintf(fp_f, "%f ", forward_satellite[i]);
    }
    // close the file
    fclose(fp_f);

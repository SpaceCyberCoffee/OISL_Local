...
void OISL_SendFile_CFDP(void)
{
    ...
    if (OISL_AppData.HkTelemetryPkt.DeviceEnabled == OISL_DEVICE_ENABLED)
    {
        ...
        // TODO IMPLEMENT INITIAL ALIGNMENT CHECK SOMEHOW. IN SEND FILE THERE ARE THE OTHERS.
        int waitCount = 0;
    
        while (!OISL_AppData.DevicePkt.Oisl.ForwardAlignment && waitCount < 60) {
             CFE_EVS_SendEvent(OISL_CMD_SEND_FILE_EID, CFE_EVS_EventType_INFORMATION,
                          "OISL FILE CFDP: the alignment condition is not verified, waiting for alignment.");
            ...
            double simTime = (double)seconds + ((double)subseconds / 4294967296.0); // 4294967296.0 = 2^32
            printf("Secndi sim %f", simTime);
            while (simTime < 100) {
                nowT = CFE_TIME_GetTime();
                seconds = nowT.Seconds;
                subseconds = nowT.Subseconds;
                simTime = (double)seconds + ((double)subseconds / 4294967296.0);
                printf("SIm time %f", simTime);
            }
            waitCount += 1;
        }
        // sendFile(fileContent, fileSize);
        // TODO IS THERE A WAY TO CALL THIS METHOD AND THEN EXIT FROM THIS FUNCTION SO THAT THE REST OF THE TM CAN BE RECEIVED?

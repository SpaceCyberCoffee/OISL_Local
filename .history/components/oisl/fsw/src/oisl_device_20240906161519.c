...
int32_t OISL_RequestData(uart_info_t* device, OISL_Device_Data_tlm_t* data)
{
    ...
    data->BackwardConnection = read_alignment_info(alignment_info_forward);

    // /* Connection with Backward Satellite */
    const char* alignment_info_forward = "/home/jstar/Desktop/github-nos3/components/oisl/fsw/src/B_sat_for_alignment.txt";
    // read the value in the file, which is 1 or 0. if the file is there and the info is 1, then forward connection =1. else, maning if the filke does not exist or the info is 0, then 0.
    data->BackwardConnection = 1 if () 0 else;

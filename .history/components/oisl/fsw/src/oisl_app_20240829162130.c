...
{
    int32 status = OS_SUCCESS;
    const char *filePath = "/home/jstar/Desktop/github-nos3/components/oisl/fsw/src/forward_sat.txt"";  // Specify the file path
    FILE *file;
    long fileSize;
    char *fileContent;
    ...
    if (OISL_AppData.HkTelemetryPkt.DeviceEnabled == OISL_DEVICE_ENABLED)
    {
        ...
        {
            OISL_AppData.HkTelemetryPkt.DeviceErrorCount++;
            CFE_EVS_SendEvent(OISL_CMD_SEND_FILE_ERR_EID, CFE_EVS_EventType_ERROR, "OISL: Unable to open file %s.", filePath);
            return;
        }

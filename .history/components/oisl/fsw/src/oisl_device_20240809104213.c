...
int32_t OISL_RequestData(uart_info_t* device, OISL_Device_Data_tlm_t* data)
{
    ...
    // Write just to test 
    char *filename = "/home/jstar/Desktop/github-nos3/components/oisl/fsw/src/ECI_backISL.txt";
    ...
    for (int i = 0; i < 3; i++) {
        fprintf(fp, "%f ", backward_isl_vector[i]);
    }
    fprintf(fp, "\n");
    // close the file
    fclose(fp);   
...
void get_isl_vectors(double* forward_ISL_vector, double* backward_ISL_vector)
{
    ...
    double ISL_vector_backward[] = {backward_satellite[0] - central_sat[0] , backward_satellite[1]central_sat[1] - , central_sat[2] - backward_satellite[2]};

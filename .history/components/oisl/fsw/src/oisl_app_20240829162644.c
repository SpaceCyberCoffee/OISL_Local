...
void OISL_SendFile_CFDP(void)
{
    ...
    if (OISL_AppData.HkTelemetryPkt.DeviceEnabled == OISL_DEVICE_ENABLED)
    {
        ...
        // TODO ESTIMATE TRANSFER TIME BASED ON THE FILE SIZE AND THE TRANSFER SPEED.

        CFE_EVS_SendEvent(OISL_CMD_SEND_FILE_EID, CFE_EVS_EventType_INFORMATION, "OISL FILE CFDP : JUst read the file with size: %d. Estimated transfer time is: TODO.", fileSize);

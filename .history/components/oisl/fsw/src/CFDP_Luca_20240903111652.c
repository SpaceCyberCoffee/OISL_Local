...
CF_CFDP_PduAck_t createAck(CF_CFDP_FileDirective_t dir_code, CF_CFDP_ConditionCode_t cc, int segmentNumber) {
    ...
    ack.directive_and_subtype_code = (dir_code << 4) | 1; // Directive and subtype code
    ack.cc_and_transaction_status = (cc << 4) | CF_CFDP_TransactionStatus_SUCCESS; // Condition code and transaction status

    // For simplicity, print ACK details
    printf("Created ACK for PDU #%d\n", segmentNumber);
    printf("Directive and subtype code: %d\n", ack.directive_and_subtype_code);
    printf("Condition code and transaction status: %d\n", ack.cc_and_transaction_status);
...
void sendAck(CF_CFDP_PduAck_t *ack) {
    ...
    printf("Sending ACK with directive and subtype code: %d, condition code and transaction status: %d\n",
           ack->directive_and_subtype_code, ack->cc_and_transaction_status);
}
...
int receiveAck(int segmentNumber) {
    ...
    if (ack->cc_and_transaction_status == CF_CFDP_TransactionStatus_SUCCESS) {
            ...
            if (OISL_AppData.DevicePkt.Oisl.ForwardAlignment == 0) {
                ...
                if (sent == 1) {
                    ...
                    if (receivePDU(&headers[segmentNumber], &contents[segmentNumber], segmentNumber)) {
                        CF_CFDP_PduAck_t ack = createAck(CF_CFDP_FileDirective_DATA, CF_CFDP_ConditionCode_NO_ERROR, segmentNumber);
                        sendAck(&ack);

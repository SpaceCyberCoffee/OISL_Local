...
void simulateNetworkDelay(void) {
    ...
    // TODO: How to implement this as 100ms sim
}
...
double estimateTransferTime(size_t fileSize, int segmentCount) {
    ...
    double delay = segmentCount * networkDelay / 1000; // seconds
    return estimatedTransferTimeSeconds + delay;
}

// CFDP-like file sending function
void sendFile(const char *fileContent, size_t fileSize) {
    int segmentNumber = 0;
    ...
    CFE_EVS_SendEvent(OISL_CMD_SEND_FILE_EID, CFE_EVS_EventType_INFORMATION,
                          "OISL FILE CFDP: Estimated Transfer time including network delay: %f s.", transferTime);

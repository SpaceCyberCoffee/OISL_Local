...
#define MIN_TORQUE_THRESHOLD 1.0E-7  // Define a small threshold to avoid negligible torque commands
...
static void AC_oisl(Generic_ADCS_GNC_Tlm_Payload_t *GNC, Generic_ADCS_AC_OISL_Tlm_t *ACS, const char *F_or_B)
{
   ...
   double SoS = 0.0;
    ...
    if ((SoS > (EPS - 1.0)) && (SoS < (EPS_OISL))) {
        ...
        if (fabs(err_b[0] - err_b[1]) < EPS_OISL && fabs(err_b[0] - err_b[2]) < EPS_OISL) {
            err_b[0] = -err_b[0];
        }

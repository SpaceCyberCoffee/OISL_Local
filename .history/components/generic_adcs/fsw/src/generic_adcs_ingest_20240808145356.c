...
void Generic_ADCS_ingest_generic_OISL(CFE_MSG_Message_t * Msg, Generic_ADCS_DI_OISL_Tlm_Payload_t *OISL)
{
    ...
    // Take these values and store them in a file 
    FILE *test_changes = fopen("/home/jstar/Desktop/github-nos3/components/oisl/fsw/src/testchanges_ISL.txt", "a");
    fprintf(test_changes, "%f %f %f\n", OISL_vector_ECI[0], OISL_vector_ECI[1], OISL_vector_ECI[2]);
    fclose(test_changes);

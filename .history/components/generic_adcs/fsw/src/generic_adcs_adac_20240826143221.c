...
static void AC_oisl(Generic_ADCS_GNC_Tlm_Payload_t *GNC, Generic_ADCS_AC_OISL_Tlm_t *ACS, const char *F_or_B)
{
   ...
   double SoS_secondary = 0.0;
    ...
    if (strcmp(F_or_B, "FORWARD") == 0) {
        ...
        SoS = SoS_F;
        target_secondary[3] = {0.0, -1.0, 0.0};
    ...
    // Check alignment with both ISL and -ISL vectors
    if ((SoS > (EPS - 1.0) && SoS < (EPS_OISL)) && (SoS_secondary > (EPS_ALIGN))) {
        VxV(selected_ISL_vector, selected_sside, ACS->therr);
    } else if (SoS >= EPS_OISL && SoS_secondary >= EPS_ALIGN) {
        // Both alignments are good
        ACS->therr[0] = 0.0;
        ACS->therr[1] = 0.0;
        ACS->therr[2] = 0.0;
    } else {
        // Handle errors for both alignments
        ...
        // Combine with the secondary axis error if needed
        // Modify ACS->therr based on secondary alignment error as needed
    }

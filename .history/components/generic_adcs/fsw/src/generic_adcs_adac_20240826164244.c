...
//    double temp_sside[3] = {0.0, 0.0, 0.0};
   ...
   double err_secondary[3] = {0.0, 0.0, 0.0};
    ...
    double magnitude_previous, magnitude_change, percentage_change;
    ...
    // Calculate the scalar product between the current and previous ISL vectors
  
    ISL_vector_change = VoV(selected_ISL_vector[i] - previous_ISL_vector[i];
    ...
    FILE *file = fopen("/home/jstar/Desktop/github-nos3/sims/build/bin/ISL_change.txt", "w");  // "a" for append
    if (file != NULL) {
        fprintf(file, "%.4f", percentage_change);
        fclose(file);
    } else {

    ...
    Generic_ADCS_GNC_Tlm_Payload_t *GNC, Generic_ADCS_AC_Tlm_Payload_t *ACS)
{
    ...
    &ACS->OISL.sside_B[0], &ACS->OISL.sside_B[1], &ACS->OISL.sside_B[2], &ACS->OISL.sside_B[0], &ACS->OISL.sside_B[1], &ACS->OISL.sside_B[2], &ACS->OISL.vmax, 
...
{
   int i;
   double u1[3] = {0.0, 0.0, 0.0}, err_b[3] = {0.0, 0.0, 0.0};      /* angle error calculation parameters */
   double temp_sside[3] = {0.0, 0.0, 0.0};
   double SoS = 0.0;
   ...
   // Calculate the scalar product between ISL vector and nadir vector
   double SoS_nadir = VoV(selected_ISL_vector, nadir);

   // Handle stabilization for the main axis (selected_sside)
   if ((SoS > (EPS - 1.0)) && (SoS < (1.0 - EPS))) {
   ...
   // Handle stabilization for the nadir vector
   double therr_nadir[3] = {0.0, 0.0, 0.0};
   if ((SoS_nadir > (EPS - 1.0)) && (SoS_nadir < (1.0 - EPS))) {
       VxV(selected_ISL_vector, nadir, therr_nadir);
   } else if (SoS_nadir >= (1.0 - EPS)) {
       therr_nadir[0] = 0.0;
       therr_nadir[1] = 0.0;
       therr_nadir[2] = 0.0;
   } else {
       ...
       VxV(nadir, err_b, temp_sside);
       VxV(selected_ISL_vector, temp_sside, therr_nadir);
   }

   // Combine errors for both axes
   for (i = 0; i < 3; i++) {
       ACS->therr[i] += therr_nadir[i];
   }

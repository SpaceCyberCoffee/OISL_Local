...
{  
   double conjugate[4];
   conjugate[0] = - DI_St->q[0];
   conjugate[1] = - DI_St->q[1];
   conjugate[2] = - DI_St->q[2];
   conjugate[3] = DI_St->q[3];
   QxV(conjugate, DI_OISL->ISL_vector_F, DI_OISL->ISL_vector_Body_F); // convert from sensor frame to body frame

...
void Generic_ADCS_ingest_generic_st(CFE_MSG_Message_t * Msg, Generic_ADCS_DI_St_Tlm_Payload_t *St)
{
    ...
    // Check if all elements of q are non-zero (within the tolerance)
    if (fabs(q[0]) > epsilon && fabs(q[1]) > epsilon && fabs(q[2]) > epsilon && fabs(q[3]) > epsilon) {
...
void Generic_ADCS_ingest_generic_OISL(CFE_MSG_Message_t * Msg, Generic_ADCS_DI_OISL_Tlm_Payload_t *OISL)
{
    ...
    double OISL_vector_ECI[3] = {oisl->Oisl.FORWARD_ISL_X, oisl->Oisl.FORWARD_ISL_Y, oisl->Oisl.FORWARD_ISL_Z};    

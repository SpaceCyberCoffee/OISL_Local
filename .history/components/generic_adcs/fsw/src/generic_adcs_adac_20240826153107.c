...
//    double temp_sside[3] = {0.0, 0.0, 0.0};
   ...
   double err_secondary[3] = {0.0, 0.0, 0.0};
    ...
    static double previous_ISL_vector[3] = {0.0, 0.0, 0.0};  // Static to retain values between function calls
    double ISL_vector_change[3] = {0.0, 0.0, 0.0};  // To store the change in ISL vector
    double magnitude_current, magnitude_previous, magnitude_change, percentage_change;
    ...
    OS_printf("AC OISL with forward vector %f %f %f", selected_ISL_vector[0], selected_ISL_vector[1], selected_ISL_vector[2]);
    ...
    // Calculate the difference between the current and previous ISL vectors
    for (i = 0; i < 3; i++) {
        ISL_vector_change[i] = current_ISL_vector[i] - previous_ISL_vector[i];
    }

    // Calculate magnitudes
    magnitude_current = vector_magnitude(selected_ISL_vector);
    magnitude_previous = vector_magnitude(previous_ISL_vector);
    magnitude_change = vector_magnitude(ISL_vector_change);

    // Avoid division by zero
    if (magnitude_previous > 0.0) {
        percentage_change = magnitude_change / magnitude_previous;
    } else {
        percentage_change = 0.0;  // No change if the previous vector is zero
    }

    // Save the percentage change to a file
    FILE *file = fopen("/home/jstar/Desktop/github-nos3/sims/build/bin/VoV_BACKWARD.txt", "a");  // "a" for append
    if (file != NULL) {
        fprintf(file, "Percentage Change: %.4f\n", percentage_change);
        fclose(file);
    } else {
        printf("Error opening file to save ISL vector changes.\n");
    }

    // Update the previous ISL vector with the current one for the next iteration
    for (i = 0; i < 3; i++) {
        previous_ISL_vector[i] = current_ISL_vector[i];
    }
    ...
    if (SoS >= EPS_OISL && SoS_secondary >= EPS_OISL) {
        for (i = 0; i < 3; i++) {
            ACS->therr[i] = 0.0;
        }
    } else {

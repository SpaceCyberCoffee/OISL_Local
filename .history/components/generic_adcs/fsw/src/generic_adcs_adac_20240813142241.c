...
static void AC_oisl(Generic_ADCS_GNC_Tlm_Payload_t *GNC, Generic_ADCS_AC_OISL_Tlm_t *ACS, const *char F_or_B)
{
   ...
   char *filename = "/home/jstar/Desktop/github-nos3/sims/build/bin/VoV_FORWARD.txt";
   FILE *fp = fopen(filename, "w");
   fprintf(fp, "%f", SoS_F);
   fclose(fp);   
   char *filename = "/home/jstar/Desktop/github-nos3/sims/build/bin/VoV_BACKWARD.txt";
   FILE *fp = fopen(filename, "w");
   fprintf(fp, "%f", SoS_B);
   fclose(fp); 
   /* Depending on the char define SoS to align with FORWARD or BACKWARD sat */ 
   if

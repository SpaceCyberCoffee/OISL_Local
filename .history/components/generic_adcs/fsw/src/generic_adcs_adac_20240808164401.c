...
{  
   ...
   OS_printf("For the conversion I am using this quaternion: %f %f %f %f", DI_St->q[0], r frame to body frame
   OS_printf("For the conversion I am using this quaternion: %f %f %f %f", , r frame to body frame
   OS_printf("For the conversion I am using this quaternion: %f %f %f %f", , r frame to body frame
...
static void AC_oisl(Generic_ADCS_GNC_Tlm_Payload_t *GNC, Generic_ADCS_AC_OISL_Tlm_t *ACS)
{
   ...
   printf("Scalar product between ISL V and desired b2 (should be close to 1): %f\n", SoS);
    ...
    fprintf(test_changes, "%f %f %f\n", GNC->ISL_vector[0], GNC->ISL_vector[1], GNC->ISL_vector[2]);
    fclose(test_changes);
   if ((SoS > (EPS - 1.0)) && (SoS < (1.0 - EPS))) {

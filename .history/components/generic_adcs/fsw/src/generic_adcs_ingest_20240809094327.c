...
void Generic_ADCS_ingest_generic_fss(CFE_MSG_Message_t * Msg, Generic_ADCS_DI_Fss_Tlm_Payload_t *Fss)
{
    ...
    if (Fss->valid == 1) {
        ...
        // Take these values and store them in a file 
        FILE *test_changes = fopen("/home/jstar/Desktop/github-nos3/components/oisl/fsw/src/testchanges_sun.txt", "a");
        fprintf(test_changes, "%f %f %f\n", svs[0], svs[1], svs[2]);
        fclose(test_changes);
    } else {
        Fss->svb[0] = 0.0;
        Fss->svb[1] = 0.0;
...
void Generic_ADCS_ingest_generic_st(CFE_MSG_Message_t * Msg, Generic_ADCS_DI_St_Tlm_Payload_t *St)
{
    ...
    // Update the value only when is valid
    if (St->valid == 1) {
    QxQ(q, St->qbs, St->q);
    }
    OS_printf("For the conversion I am using this quaternion: %f %f %f %f\n", St->q[0], St->q[1], St->q[2], St->q[3]);
    // OS_printf("What if I use these ? Should be the ones in ST ref frame %f %f %f %f\n", q[0], q[1], q[2], q[3]); yes they are
    OS_printf("TODO: find a way to get the quaternion from 42 and use that for the conversion");
}
...
void Generic_ADCS_ingest_generic_OISL(CFE_MSG_Message_t * Msg, Generic_ADCS_DI_OISL_Tlm_Payload_t *OISL)
{
    ...
    double OISL_vector_ECI[3] = {oisl->Oisl.BACKWARD_ISL_X, oisl->Oisl.BACKWARD_ISL_Y, oisl->Oisl.BACKWARD_ISL_Z};    // SHOULD BE SIMILAR TO VELOCITY UNITY VECTOR since it is backward-central so direction of the forward.
